...
def check_minervini_trend_template_price(stock_data):
    """
    Returns condition results for Price only.
    """
    results = {}
    results['price_vs_sma'] = stock_data['price'] > stock_data['sma150'] and stock_data['price'] > stock_data['sma200']
    results['sma_trend'] = stock_data['sma150'] > stock_data['sma200']
    results['short_term_strength'] = stock_data['sma50'] > stock_data['sma150'] and stock_data['sma50'] > stock_data['sma200']
    results['price_vs_sma50'] = stock_data['price'] > stock_data['sma50']
    results['above_low'] = stock_data['price'] >= stock_data['low_52week'] * 1.3
    results['near_high'] = stock_data['price'] >= stock_data['high_52week'] * 0.75
    return results


def check_minervini_trend_template_full(stock_data):
    """
    Returns condition results for both Price and RS line.
    Final pass/fail is left to external logic (e.g., Google Sheets).
    """
    results = {}
    ...
    # --- RS LINE CONDITIONS ---
    results['rs_vs_sma'] = stock_data['rs_line'] > stock_data['rs_sma150'] and stock_data['rs_line'] > stock_data['rs_sma200']
    results['rs_sma_trend'] = stock_data['rs_sma150'] > stock_data['rs_sma200']
    results['rs_short_term_strength'] = stock_data['rs_sma50'] > stock_data['rs_sma150'] and stock_data['rs_sma50'] > stock_data['rs_sma200']
    results['rs_vs_sma50'] = stock_data['rs_line'] > stock_data['rs_sma50']
    results['rs_above_low'] = stock_data['rs_line'] >= stock_data['rs_low_52week'] * 1.3
    results['rs_near_high'] = stock_data['rs_line'] >= stock_data['rs_high_52week'] * 0.75
